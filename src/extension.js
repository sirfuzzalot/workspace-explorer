...
const activate = async () => {
	...
	try {
		...
		vscode.commands.registerCommand("workspaceExplorer.openWorkspaceInNewWindow", (context) => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.openWorkspaceInNewWindowQuickPick", async () => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.openWorkspaceInSameWindowQuickPick", async () => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.openWorkspaceInSameWindow", (workspaceFileNameAndFilePath) => {
			try {
				vscode.commands.executeCommand("vscode.openFolder", vscode.Uri.file(workspaceFileNameAndFilePath), false);
			} catch (err) {
				vscode.window.showErrorMessage(err);
			}
		});
		...
		vscode.commands.registerCommand("workspaceExplorer.openWorkspaceExplorerStorageDirectory", () => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.addSubFolder", async (context) => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.deleteFolder", async (context) => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.refreshWorkspaceExplorer", () => {
			treeDataProvider.refresh();
		});

		// Register Change Folder icon Command.
		vscode.commands.registerCommand("workspaceExplorer.changeFolderIcon", async (e) => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.changeWorkspaceIcon", async (e) => {
			try {
				await changeIcon(e, treeDataProvider);
			} catch (err) {
				vscode.window.showErrorMessage(err);
		...
		vscode.commands.registerCommand("workspaceExplorer.createWorkspace", async (e) => {
			try {
		...
		vscode.commands.registerCommand("workspaceExplorer.deleteWorkspace", async (e) => {
			try {
		...
		console.log(`[vscode-workspace-explorer] ${extensionVersion} ==> Activated`);
	} catch (err) {
		vscode.window.showErrorMessage(err);
	}
};
