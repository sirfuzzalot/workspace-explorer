...
function promisify(fn) {
    return function promisified(...params) {
      return new Promise((resolve, reject) => fn(
          ...params.concat([(err, ...args) => err ? reject(err) :
             resolve( args.length < 2 ? args[0] : args )])))
    }
}

/*
Gets an async versino of fs.readdir
*/
...
*/
const findChildren = function(workspaceStorageDirectory) {
...
*/
const getCustomWorkspaceIcons = function(
    workspaceFileNameAndFilePath,
    collapsableState,
    extensionConfig){
...
*/
const getDefaultWorkspaceIcons = function(collapsableState){
    if (collapsableState == vscode.TreeItemCollapsibleState.Collapsed){
        return {
            light: path.join(
                path.dirname(__filename),
...
*/
const sortFilesAndFolders = function(a,b){
    if (a.collapsableState == vscode.TreeItemCollapsibleState.Collapsed){
        if (b.collapsableState == vscode.TreeItemCollapsibleState.Collapsed){
            let sortingArray = [a.label, b.label]
            sortingArray.sort()
